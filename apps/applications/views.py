...
def resend_notifications_view(request, application_id):
    """View для повторной отправки уведомлений для конкретной заявки."""
    try:
        application = Application.objects.get(id=application_id)
        from apps.applications.tasks import send_application_notifications
        send_application_notifications.delay(application.id)
        messages.success(request, f"Уведомления для заявки {application.application_number} успешно отправлены в очередь.")
    except Application.DoesNotExist:
        messages.error(request, f"Заявка с ID {application_id} не найдена.")
    except Exception as e:
        logger.error(f"Ошибка при повторной отправке уведомлений для заявки {application_id}: {e}")
        messages.error(request, f"Ошибка при повторной отправке уведомлений: {e}")

    # Перенаправляем обратно на список заявок в админке
    return redirect(reverse('admin:applications_application_changelist'))
