...
from django.urls import reverse, path # Добавлено path
from django.utils.safestring import mark_safe
from .models import Application, ApplicationStatus
from .views import resend_notifications_view # Импортируем новую view-функцию
from django.contrib import messages # Добавлено messages
...
class ApplicationAdmin(admin.ModelAdmin):
    ...
    # Добавляем кастомные URL-ы для этой модели в админке
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path(
                '<int:application_id>/resend-notifications/', # URL-паттерн
                self.admin_site.admin_view(resend_notifications_view), # View-функция
                name='applications_application_resend_notifications', # Имя для reverse
            ),
        ]
        return custom_urls + urls # Важно добавить кастомные URL-ы перед стандартными
    ...
    def admin_actions(self, obj):
        # Используем правильное имя для reverse, которое мы определили в get_urls
        resend_url = reverse('admin:applications_application_resend_notifications', args=[obj.pk])
        return format_html(
